...
export const userService = {
  async createUser(userData: CreateUserData): Promise<void> {
    try {
      ...
      const filteredData = Object.fromEntries(
        Object.entries(userDataToSave).filter(
      ...
      console.warn(
        "Firestore createUser failed, using localStorage fallback:",
        error,
      );
  ...
  async getUserProfile(uid: string): Promise<UserProfile | null> {
    try {
      ...
      return {
        ...
        gameHistory:
          data.gameHistory?.map((game: Record<string, unknown>) => ({
            ...game,
            timestamp:
              (game.timestamp as { toDate: () => Date })?.toDate() ||
              new Date(),
          })) || [],
      } as UserProfile;
    } catch (error) {
      console.warn(
        "Firestore getUserProfile failed, checking localStorage:",
        error,
      );
  ...
  async updateUserProfile(
    ...
    try {
      ...
      console.warn(
        "Firestore updateUserProfile failed, updating localStorage:",
        error,
      );
      const storedProfile = localStorage.getItem(`user_profile_${uid}`);
      if (storedProfile) {
  ...
  async updateCurrentGame(
    uid: string,
    gameData: UserProfile["currentGame"],
  ): Promise<void> {
    const userRef = doc(db, USERS_COLLECTION, uid);
    await updateDoc(userRef, {
